...
void Render()
{
	...
	if (menuShow)
	{
		...
		bool ret = LoadTextureFromFile("C:/rickroll.jpg", &my_texture, &my_image_width, &my_image_height);
		IM_ASSERT(ret);
		auto draw = ImGui::GetWindowDrawList();

		ImGui::Image((void*)my_texture, ImVec2(my_image_width, my_image_height));

		bBlur->blur_background(draw, DirectX9Interface::pDevice);
...
int main()
{
	_HWND = FindWindow(NULL, L"WindowName");
